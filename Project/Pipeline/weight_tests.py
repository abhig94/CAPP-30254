...
modelLRAB  = {'model': AdaBoostRegressor, 'n_estimators': [25, 50]}#, 200]}
Adareg = [modelLRAB]
...
if __name__ == '__main__':
  ...
  if not ens:
    if not mac:
      if not b:
        results[0] = clf_loop_reloaded(x,y,5,cluster_test,to_discretize, 10, weights, True, True, False)
        namelist[0] += 'truetrue'
        results[1] = clf_loop_reloaded(x,y,5,cluster_test,to_discretize, 10, weights, False, True, False)
        namelist[1] += 'falsetrue'
        results[2] = clf_loop_reloaded(x,y,5,cluster_test,to_discretize, 10, weights, True, False, False)
        namelist[2] += 'truefalse'
        results[3] = clf_loop_reloaded(x,y,5,cluster_test,to_discretize, 10, weights, False, False, False)
        namelist[3] += 'falsefalse'


      else:
        x = x.drop(drops, 1)
        results = clf_loop_reloaded(x,y,5,Adareg,to_discretize, 10, weights, True, True, False)
    else:
      results = clf_loop_reloaded(x,y,5,Adareg,to_discretize, 10, weights, True, True, True)

  else:
    if not b and not mac:
      results = clf_loop_revolutions(x, y, 5, Adareg, to_discretize, 10, weights, True, False)
    elif not b and mac:
      results = clf_loop_revolutions(x, y, 5, Adareg, to_discretize, 10, weights, True)
    else: #blind and macro
      x = x.drop(drops, 1)
      results = clf_loop_revolutions(x, y, 5, Adareg, to_discretize, 10, weights, True)

  os.chdir('Results')
  os.chdir('Weight_tests')
  if RF:
    makeTmpFile(name)
  for name, res in zip(namelist, results):
    name += '.csv'
    write_results_to_file(name, res)
