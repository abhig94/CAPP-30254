...
def getBestByRegion():
	...
	l = []
	for f in fList:
		if 'comparison' not in f and 'best' not in f and 'ensemble' in f and 'cluster' not in f:
			l.append(appendRegions(f))

def appendRegions(fName):
	data = readFile(fName)
	regIndx = data[0].index('Subsection')
	aucIndx = data[0].index('AUC')
	classifIndx = data[0].index('classifier')
	dLen = len(data)
	regions = set()
	d = {}
	for j in range(1, dLen):
		...
		if key != '' and key != ' ':
			num = d.get(key, (0,0))
			rowAUC = data[j][aucIndx]
			if num[0] < rowAUC:
				d[key] = (rowAUC, getName(data[j], classifIndx))
	return (d, fName)
